! function($, window, e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 'success') {
                    ...
                    if (request['demo-data-package'] == 0){
    ...
    var actionInstallPackage    =   function (request, button, dialogPopup, index, data) {
        if (index<data.extensions.length) {
            var extension   =   data.extensions[index];
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == 'success') {
                        progressbar.attr('aria-valuenow',((index+1)*100)/data.extensions.length).css('width', ((index+1)*100)/data.extensions.length+'%');
        ...
        if ($('#astroid-form-fieldset-section-jollyanyimport .astroid-form-fieldset-section').length) {
            ...
            $('.intall-package').on('click', function (e) {
                ...
                if (thisLink.data('status') == 0) {
                    alert('You need activate your license to use this premium feature!');
                } else {
                    dialogPopup.find('.package-name').text(thisLink.data('name'));
                    dialogPopup.find('.install-action').attr('data-file',$(this).data('file'));
            ...
            $('.install-action').on('click', function (e) {
                ...
                request['option']               = 'com_ajax';
                request['extension-package']    = 1;
                request['demo-data-package']    = 0;
            ...
            dialogPopup.on('hidden.bs.modal', function (e) {
                $(this).find('.install-action').attr('data-file','');

...
$css_button = 'uk-button uk-button-default uk-margin-small-top '. $this->config->get('css_button', 'hikabtn');
...
if($waitlist_btn) {
?>
	<a class="<?php echo $css_button; ?>" rel="nofollow" href="<?php echo hikashop_completeLink('product&task=waitlist&cid='.$this->row->product_id.$this->itemid); ?>"><span><?php
...
?>
	<form action="<?php echo hikashop_completeLink($classical_url); ?>" method="post" class="uk-margin-small-top" name="hikashop_product_form_<?php echo $this->row->product_id.'_'.$this->params->get('main_div_name'); ?>" enctype="multipart/form-data">
<?php
}
?>
...
<!-- ADD TO CART BUTTON -->
<?php
if($add_to_cart && $add_to_wishlist && !$has_options && !$global_on_listing) {
    echo '<div class="uk-button-group">';
}
if($add_to_cart && !$has_options && !$global_on_listing) {
?>
	<a class="jollyany-hikashop-atc <?php echo $css_button . ' ' . $css_button_cart; ?>" rel="nofollow" href="<?php echo hikashop_completeLink($classical_url); ?>" onclick="if(window.hikashop.addToCart) { return window.hikashop.addToCart(this); }" data-addToCart="<?php echo $this->row->product_id; ?>" data-addTo-div="<?php echo $this->params->get('main_div_name'); ?>"<?php if(!empty($extra_div_name)){ echo ' data-addTo-extra="' . $extra_div_name. '"'; } ?> data-addTo-class="add_in_progress"><span><?php
...
?>
	<a class="<?php echo $css_button . ' ' . $css_button_wishlist. (empty($this->row->product_addtowishlist_message) ? ' uk-padding-small uk-padding-remove-vertical' : ''); ?>" rel="nofollow" href="<?php echo hikashop_completeLink($classical_url.'&cart_type=wishlist'); ?>" onclick="if(window.hikashop.addToWishlist) { return window.hikashop.addToWishlist(this); }" data-addToWishlist="<?php echo $this->row->product_id; ?>" data-addTo-div="<?php echo $this->params->get('main_div_name'); ?>"<?php if(!empty($extra_div_name)){ echo ' data-addTo-extra="' . $extra_div_name. '"'; } ?> data-addTo-class="add_in_progress"><span><?php
		if(!empty($this->row->product_addtowishlist_message))
			echo JText::_($this->row->product_addtowishlist_message);
		else
			echo '<span data-uk-icon="heart"></span>';
	?></span></a>
<?php
}
if($add_to_cart && $add_to_wishlist && !$has_options && !$global_on_listing) {

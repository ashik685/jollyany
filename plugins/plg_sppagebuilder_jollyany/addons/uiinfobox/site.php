...
class SppagebuilderAddonUiInfoBox extends SppagebuilderAddons {
	...
	public function render() {
		...
		$output .= '<div class="ui-info-box' . $zindex_cls . $general . $max_width_cfg . '"' . $animation . '>';
		...
		$output .= '<div class="uk-grid-match ' . $text_alignment . $grid . '" uk-grid="' . $masonry_cls . $gallery_parallax . '">';
		...
		if ( isset( $settings->ui_grid_item ) && count( (array) $settings->ui_grid_item ) ) {
			foreach ( $settings->ui_grid_item as $key => $value ) {
				...
				$icon_name    = ( isset( $value->icon_name ) && $value->icon_name ) ? $value->icon_name : '';
	...
	public function css() {
		...
		foreach ( $settings->ui_grid_item as $key => $addon_item ) {
			...
			if ( $icon_size && $media_type != 'uikit_icon' ) {
				$css .= $addon_id . ' .tz-custom-icon span,' . $addon_id . ' .tz-custom-icon i,' . $addon_id . ' .uk-icon-link {';
